...
bool Output::on_chunk(audio_chunk *const chunk, abort_callback &)
{
	...
	try
	{
		...
		if (!chunk->to_raw_data(buf, bps) || output->canWrite() < buf.get_size() || !output->write((const byte_t *)buf.get_ptr(), buf.get_size()))
		{
						...
						win32_event &complete)
{
	try
	{
		...
		if (plm->get_playing_item_location(&ppl, &idx) && ppl == plm->get_playing_playlist() // alt api
			&& (len = plm->playlist_get_item_count(ppl)) > 0)
		{
			...
			{
				listpos = std::make_pair(static_cast<short>(idx + 1), static_cast<short>(len));
				goto end;
			}
			// try next item in the playlist (assumes normal play order)
					 ...
					 itm.is_valid() && itm == mdb)
			{
				listpos = std::make_pair(static_cast<short>(idx + 2), static_cast<short>(len));
				goto end;
			}
		}

		if ((ppl = plm->get_playing_playlist()) != -1 && (len = plm->playlist_get_item_count(ppl)) > 0)
		{
...
OutputMetadata Output::to_metadata(const metadb_handle_ptr &mdb) const
{
	...
	if (mdb->get_info_locked(inf))
	{
		...
		if (inf->meta_exists("title"))
			title = inf->meta_get("title", 0);
		if (inf->meta_exists("album"))
			album = inf->meta_get("album", 0);
		if (inf->meta_exists("artist"))
			artist = inf->meta_get("artist", 0);
	}

	if (title.empty())
	{
		const char *const path = mdb->get_path();
		char file[MAX_PATH];
	...
	try
	{
		...
		if (aad.get_ptr() != NULL && aad->get_size() > 10 && std::memcmp((const byte_t *)aad->get_ptr() + 6, "JFIF", 4) == 0)
		{
