...
static std::string part_one_of(const std::string& composite)
{
	return composite.substr(0, composite.find_first_of('@'));
}


static std::string part_two_of(const std::string& composite)
{
	return composite.substr(composite.find_first_of('@') + 1);
}
	...
	private Uncopyable
{
	...
	std::map<const std::string,const std::string> _secrets;
...
const std::string& DeviceDiscovery::getDeviceAuth(const DeviceInfo& deviceInfo)
{
	const std::string id = part_one_of(deviceInfo.addr().first);
	return impl()._secrets[id];
}
...
void DeviceDiscoveryImpl::addListener(DeviceDiscovery::Listener& listener)
{
	...
	{
		_devices.clear(); _secrets.clear(); _services.clear();
	...
	const uint32_t ttl)
{
	...
	const DeviceInfo device(
		determineDeviceType(txtRecord), part_two_of(info.name),
		std::make_pair(info.name, info.type), /*zeroConf:*/true);
	...
	if (txtRecord.has("pk") && txtRecord.has("vs") && txtRecord.test("vs", "[3-9]\\d{2}.*"))
	{
		const std::string pk = txtRecord.get("pk");
		if (pk.length() == 64)
		{
			const std::string id = part_one_of(info.name);
			_secrets.insert(std::make_pair(id, pk));
		}
	}
	...
	const ServiceDiscovery::TXTRecord& txtRecord) const
{
	...
	// 7.6.7: txtvers=1 sr=44100 ss=16 ch=2 cn=0,1 ek=1 et=0,1 da=true pw=false sv=false tp=TCP,UDP vn=65537 vs=105.1 am=AirPort4,107 fv=76700.5  sf=0x5
	// 7.6.8: txtvers=1 sr=44100 ss=16 ch=2 cn=0,1 ek=1 et=0,1 da=true pw=false sv=false tp=TCP,UDP vn=65537 vs=105.1 am=AirPort4,107 fv=76800.1  sf=0x5
	// 7.6.9: txtvers=1 sr=44100 ss=16 ch=2 cn=0,1 ek=1 et=0,1 da=true pw=false sv=false tp=TCP,UDP vn=65537 vs=105.1 am=AirPort4,107 fv=76900.11 sf=0x5
	// 7.8.1: txtvers=1 sr=44100 ss=16 ch=2 cn=0,1 ek=1 et=0,1 da=true pw=false sv=false tp=TCP,UDP vn=65537 vs=105.1 am=AirPort4,107 fv=78100.3  sf=0x5
	...
	// 7.6.7: txtvers=1 sr=44100 ss=16 ch=2 cn=0,1      et=0,4      da=true pw=false sv=false tp=TCP,UDP vn=65537 vs=105.1 am=AirPort10,115 fv=76700.5      sf=0x5
	// 7.6.8: txtvers=1 sr=44100 ss=16 ch=2 cn=0,1      et=0,4      da=true pw=false sv=false tp=TCP,UDP vn=65537 vs=105.1 am=AirPort10,115 fv=76800.1      sf=0x5
	// 7.6.9: txtvers=1 sr=44100 ss=16 ch=2 cn=0,1      et=0,4      da=true pw=false sv=false tp=TCP,UDP vn=65537 vs=105.1 am=AirPort10,115 fv=76900.11     sf=0x5
	// 7.8.0:                               cn=0,1      et=0,4 md=2 da=true                   tp=UDP     vn=65537 vs=366.0 am=AirPort10,115 fv=p20.78000.12 sf=0x4 ft=0x445D0A00,0x1C340 pk=[0-9a-f]{64}
	// 7.8.1:                               cn=0,1      et=0,4 md=2 da=true                   tp=UDP     vn=65537 vs=366.0 am=AirPort10,115 fv=p20.78100.3  sf=0x4 ft=0x445D0A00,0x1C340 pk=[0-9a-f]{64}
	...
	// 7.0.x:                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=211.2  vv=2 am=AppleTV3,[12] sf=0x44 ft=0x5A7FFFF7,0x1E pk=[0-9a-f]{64}
	// 7.1  :                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=211.3  vv=2 am=AppleTV3,[12] sf=0xc4 ft=0x5A7FFFF7,0x1E pk=[0-9a-f]{64}
	// 7.2  :                                                                                                          vs=220.68                            ^ also seen c,c4,4c,24c here for ATV 3,2

	// Apple TV (4th Gen) TXT record looks like:
	// 9.1   :                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=268.1     vv=2 am=AppleTV5,3           sf=0x44    ft=0x5A7FFFF7,0xDE       pk=[0-9a-f]{64}
	// 9.2   :                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=280.32.2  vv=2 am=AppleTV5,3           sf=0x44    ft=0x5A7FFFF7,0xDE       pk=[0-9a-f]{64}
	// 10.0  :                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=301.44.3  vv=2 am=AppleTV5,3           sf=0x44    ft=0x5A7FFFF7,0x4DE      pk=[0-9a-f]{64}
	// 10.1  :                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=310.17    vv=2 am=AppleTV5,3           sf=0x44    ft=0x5A7FFFF7,0x4DE      pk=[0-9a-f]{64}
	// 11.0  :                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=352.18.1  vv=2 am=AppleTV5,3 ov=11.0   sf=0x244   ft=0x5A7FFFF7,0x5FDE     pk=[0-9a-f]{64}
	//                                                                                                                                                                 ^ 0 also seen here
	// 11.1  :                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=353.50    vv=2 am=AppleTV5,3 ov=11.1   sf=0x244   ft=0x5A7FFFF7,0x5FDE     pk=[0-9a-f]{64}
	// 11.2  :                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=356.19    vv=2 am=AppleTV5,3 ov=11.2.5 sf=0x244   ft=0x5A7FFFF7,0x55FDE    pk=[0-9a-f]{64}
	// 11.4  :                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=366.51.3  vv=2 am=AppleTV5,3 ov=11.4   sf=0x244   ft=0x5A7FFFF7,0x155FDE   pk=[0-9a-f]{64}
	// 11.4.1:                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=366.75.2  vv=2 am=AppleTV5,3 ov=11.4.1 sf=0x244   ft=0x5A7FFFF7,0x155FDE   pk=[0-9a-f]{64}
	// 12.1  :                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=373.8     vv=2 am=AppleTV5,3 ov=12.1   sf=0x10244 ft=0x5A7FFFF7,0x155FDE   pk=[0-9a-f]{64}
	// 12.1.2:                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=375.3     vv=2 am=AppleTV5,3 ov=12.1.2 sf=0x30244 ft=0x5A7FFFF7,0x155FDE   pk=[0-9a-f]{64}
	// 13.0  :                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=406.31.82 vv=2 am=AppleTV5,3 ov=13.0   sf=0x10244 ft=0x5A7FFFF7,0x3C155FDE pk=[0-9a-f]{64}
	// 13.3  :                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=415.3     vv=2 am=AppleTV5,3 ov=13.3   sf=0x10244 ft=0x5A7FFFF7,0x3C155FDE pk=[0-9a-f]{64}
	// 13.4  :                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=420.45    vv=2 am=AppleTV5,3 ov=13.4   sf=0x10244 ft=0x5A7FDFD5,0x3C155FDE pk=[0-9a-f]{64}
	//                                                                                                                                                                  ^^ also seen 86
	// 14.2  :                               cn=0,1,2,3 et=0,3,5 md=0,1,2 da=true                   tp=UDP     vn=65537 vs=525.38.42 vv=2 am=AppleTV5,3 ov=14.2   sf=0x244   ft=0x5A7FDFD5,0x3C155FDE pk=[0-9a-f]{64}

	// Denon AVC TXT record looks like:
	//                               cn=0,1 et=0,4 md=0,1,2 da=true                   tp=UDP vn=65537 vs=366.0 am=AVC-X6500H fv=p20.1.544.240 ft=0x445F8A00,0x1C340 sf=0x4 pk=[0-9a-f]{64}
	...
	//                               cn=0,1 et=0,4 md=0,1,2 da=true                   tp=UDP vn=65537 vs=366.0 am=AVR-S730H fv=p20.1.493.180 ft=0x445F8A00,0x1C340 sf=0x4 pk=[0-9a-f]{64}
	...
	// Onkyo R-N855 ECE830 TXT record looks like:
	//                               cn=0,1 et=0,4 md=0,1,2 da=true                   tp=UDP vn=65537 vs=366.0 am=Onkyo R-N855 Network Receiver fv=p20.3.06/20124ANA ft=0x445F8A00,0x1C340 sf=0x4 pk=[0-9a-f]{64}
	...
	// Yamaha TSR-700 record looks like:
	//                               cn=0,1 et=0,4 md=0,1,2 da=true                   tp=UDP vn=65537 vs=366.0 am=TSR-700 fv=p20.1.23 ft=0x445F8A00,0x1C340 sf=0x4 pk=[0-9a-f]{64}
	...
	// Cambridge Audio 851N TXT record looks like:
	//                               cn=0,1 et=0,4 md=0,1,2 da=true                   tp=UDP vn=65537 vs=366.0 am=851N fv=p20.18.0.75 ft=0x445F8A00,0x1C340 sf=0x4 pk=[0-9a-f]{64}
	...
	// KEF LSX TXT record looks like:
	//                               cn=0,1 et=0,4 md=2     da=true                   tp=UDP vn=65537 vs=366.0 am=LSX fv=p20.4101909240.105243422 ft=0x445D0A00,0x1C340 sf=0x4 pk=[0-9a-f]{64}
	...
	// Sonos One TXT record looks like:
	//                               cn=0,1 et=0,4 md=0,1,2 da=true                   tp=UDP vn=65537 vs=366.0 am=One     fv=p20.55.1-74250 ft=0x445F8A00,0x1C340 sf=0x4 pk=[0-9a-f]{64}
	...
	// Airfoil Speakers/Satellite TXT record looks like:
	...
	else if (txtRecord.has("am") && txtRecord.test("am", "AppleTV.*") && !txtRecord.has("ek"))
	{
		...
		assert(txtRecord.has("md") && txtRecord.get("md") == "0,1,2");
		assert(txtRecord.has("sf") && txtRecord.test("sf", "0x[0-9c]{1,5}"));
		assert(txtRecord.has("tp") && txtRecord.test("tp", "(TCP,)?UDP"));
		assert(txtRecord.has("vn") && txtRecord.get("vn") == "65537");
		assert(txtRecord.has("vs") && txtRecord.test("vs", "\\d{3}\\.\\d{1,2}(\\.\\d{1,2})?"));

		assert(!txtRecord.has("ft") || txtRecord.test("ft", "0x[0-9A-F]{7,8}(,0x[0-9A-F]{1,8})?"));
		assert(!txtRecord.has("ov") || txtRecord.test("ov", "\\d{2}\\.\\d+(\\.\\d+)?"));
		assert(!txtRecord.has("pk") || txtRecord.test("pk", "[0-9a-f]{64}"));
		assert(!txtRecord.has("vv") || txtRecord.test("vv", "1|2"));
	...
	else if (txtRecord.has("am") && txtRecord.test("am", "(PIONEER|DENON|Onkyo|AV[CR]|(?:CX|851)N|DNP|HTR|JB2|NR|RX-|TSR|YH[AT]).*")
