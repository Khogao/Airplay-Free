...
public:
	 RAOPDevice(class RAOPEngine&, const std::string& authorizationKey,
		int encryptionType, byte_t metadataFlags);
	~RAOPDevice();
...
private:
	...
	const int _encryptionType;

	/** type(s) of playback metadata the device accepts */
	const byte_t _metadataFlags;
	...
	/** device's RTSP "pk" hexadecimal string */
